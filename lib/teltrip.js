...
// subscriberCost is a ONE-TIME price read from package fields (not summed weekly)
...
// ---------------- Packages (read one-time cost) ----------------
function pickOneTimeSubscriberCost(pkg, tpl) {
  // Try common/likely fields in package or template
  const candidates = [
...
async function fetchPackagesFor(subscriberId) {
  ...
  return {
    ...
    subscriberOneTimeCost: pickOneTimeSubscriberCost(p, tpl) // NEW
  };
}
...
async function fetchUsageWindow(subscriberId, startYMD, endYMD) {
  // subscriberUsageOverPeriod { subscriber:{subscriberId}, period:{start,end} } (≤1 week)
...
export async function fetchAllData(accountIdParam) {
  ...
  const rows = subscribers.map((s) => {
    ...
    return {
      ...
      pckdatabyte: null,
      useddatabyte: null,
      subscriberOneTimeCost: null, // NEW

      // totals since 2025-06-01 (usage + reseller cost only)
  ...
  await pMap(rows, async (r) => {
    ...
    // packages → includes one-time subscriber cost
    try {
      const pkg = await fetchPackagesFor(r._sid);
      if (pkg) Object.assign(r, pkg);
    } catch {}

    // aggregated usage & reseller cost (Jun 1 → today)
    try {
