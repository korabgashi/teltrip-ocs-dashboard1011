...
export default function Page() {
  ...
  // Columns incl. weekly usage (W1 = most recent week)
  const columns = [
  ...
  return (
    <main style={{ padding: 24, maxWidth: 1800, margin: "0 auto" }}>
      <header style={{ display: "grid", gridTemplateColumns: "auto auto auto 1fr 260px", gap: 12, alignItems: "center", marginBottom: 14 }}>
        ...
        />
        <button onClick={load} disabled={loading} style={{ padding:"8px 14px", borderRadius:10, border:"1px solid #2a3353", background:"#151a2e", color:"#e9edf5", cursor:"pointer" }}>
          ...
          {filtered.map((r, i) => {
            ...
            return (
              <>
                ...
                {/* weekly usage W1..W4 (most recent first) */}
                <div style={base}>{bytesToGB(r.usageW1)}</div>
                <div style={base}>{bytesToGB(r.usageW2)}</div>
                <div style={base}>{bytesToGB(r.usageW3)}</div>
                <div style={base}>{bytesToGB(r.usageW4)}</div>
              </>
            );
          })}
        </div>
      ...
      <p style={{ opacity:.7, marginTop:10, fontSize:12 }}>
        W1 = most recent week, then W2–W4 (one-week windows per API). Scroll sideways to see all columns.
      </p>
    </main>
  );
}
